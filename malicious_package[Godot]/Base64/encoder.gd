...
func _ready():
	...
	# Open the payload file and encode in Base645
	var file = FileAccess.open("res://calc.exe", FileAccess.READ)
	var buffer = file.get_buffer(file.get_length())
	var b64 = Marshalls.raw_to_base64(buffer)
		
	var bits = []

	# Convert Base64 to bits + delimiter (0x00)
	for c in b64.to_utf8_buffer():  # Godot 4.x
		for i in range(8):
			bits.append((c >> (7 - i)) & 1)
